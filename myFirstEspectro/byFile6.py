...
ax2.set_ylim(-30, 0) if RELATIVE_PER_FRAME else ax2.set_ylim(-100, 0)
...
with sd.OutputStream(samplerate=SR,
    ...
    while plt.fignum_exists(fig1.number) and plt.fignum_exists(fig2.number):
        ...
        # ----- FFT y espectro línea -----
        Xw = np.fft.rfft(frame * window)

        if NORM_MODE == "psd":
            # Welch-like PSD (calibrada)
            U = (window**2).mean()                         # potencia media de la ventana
            ps = (np.abs(Xw)**2) / (N**2 * U)              # potencia por bin (double-sided)
            ps[1:-1] *= 2.0                                # una sola cara
            mag_for_line = np.sqrt(ps)                     # para dibujar línea (raíz de potencia)
            db_line = 20*np.log10(mag_for_line + EPS)
        else:
            # Simple (magnitud de FFT)
            mag = np.abs(Xw)
            db_line = 20*np.log10(mag + EPS)

        line.set_ydata(db_line)

        # ----- Bandas ISO -----
        band_vals_db = []
        if NORM_MODE == "psd":
            # Integra potencia por banda: sum(PSD_bin) * df   -> dB
            ...
            # Promedio de potencia a partir de magnitud
            mag = np.abs(Xw)
            for idx in band_bins:
                if len(idx) < MIN_BINS_PER_BAND:
                    band_vals_db.append(-120.0)
        ...
        # Normalización relativa por frame (opción A)
        if RELATIVE_PER_FRAME:
            band_vals_db = band_vals_db - np.max(band_vals_db)

        # Actualiza barras
        for b, h in zip(bars, band_vals_db):
